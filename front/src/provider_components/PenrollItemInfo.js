...
import './PenrollItemInfo.css';
import {Link} from 'react-router-dom';

class PenrollItemInfo extends Component{
    state={
        id: 1524,
        item_id : 1,
        providerId:'',
    ...
    render(){
        const{id, providerId, name, discription, tag, price, registrationDate, photo,orderdate,expired,address} = this.state;

        return(
            <div className={`info-wrapper ${ expired ? 'expiredbox' : '' }`}>
                <div className={`info-box ${ expired ? 'expiredbox' : '' }`}>
                    <div className = "info-photo">
                        <img src={photo}/>
                    </div>
                    <div className = "Right">
                        <div className = "Info">
                            <div className = "info-item-name">[{name}]{expired ? '-만료됨' : ''}</div>
                        ...
                        <div className = {`purchase-button-2 ${ expired ? 'expired' : '' }`} onClick={this.handleCancelorder}>
                            등록취소
                        </div></Link>
...
export default PenrollItemInfo;